...
SThick = 6.35e-3 #m
######User input Type##############################
InType = ['PPM', 'Activity', 'Efficiency', 'Signal Rate', 'BGR/day']
Iso = [['U238', 'Th232', 'K40'], #[[PMT], 
       ...
       ['Fast neutrons', 'Radionuclides']] #[Cosmic muon background]
...
IsoDefault = [[0.043, 0.133, 16], #[[PMT],
              ...
              [0.02, 0.01]] #[Cosmic muon background in events per day]
######Components###################################
Comp = ['PMT', 'VETO', 'TANK', 'CONCRETE', 'ROCK','WATER', 'GD', 'COSMIC MUON']
######menu func####################################
def menu(): #menu text
    """
    Displays options
...
def BGRate():
    ...
    print('##################################################') 
    print('BGR due to Cosmic Muon Background')
    CMBGR = 0
    for i in range(len(Iso[7])):
        CMBGR += IsoDefault[7][i]
        print('BGR due to ' + Iso[7][i] + ' %.5e' % IsoDefault[7][i]) 
###################################################
    #Total#########################################
    tot = PMTBGR + VETOBGR + TANKBGR + CONCBGR + ROCKBGR + WATERBGR + GDBGR + CMBGR
...
####Cleanliness budget calculation#################
        ...
        VETOIsoAct = revVETOAct(VETOBGIso,VETOIsoEff)
